...
function generateEmployee(empArr){
  ...
  //for each employee in the team 
    empArr.forEach(employee =>{
        ...
        console.log(empRole)
        output = output.concat(generateCard(employee,empRole)); // Generate a card layout for every employee
...
function generateHTML (empArr){
  ...
  console.log("*****final data : generate HTML ***",data);
  return data;
...
return `
  ...
  {  
    ...
    <footer class="container-fluid bg-info text-center py-3">
      <h3 class="text-dark">&copy;  by Deepa Krishnan</h3>
    </footer>  
